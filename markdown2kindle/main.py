...
def md_replace_images_with_links(md_text):
    # Replace ![alt](url) with > **Image:** [alt](url)
    return re.sub(r'!\[([^\]]*)\]\(([^)]+)\)', r'> **Image:** [\1](\2)', md_text)

def convert_markdown_to_html(filepath):
    with open(filepath, 'r', encoding='utf-8') as f:
        markdown_content = f.read()
    markdown_content = md_replace_images_with_links(markdown_content)
